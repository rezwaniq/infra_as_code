...
def getInputData():
    with open("aws_data.yaml", 'r') as stream:
        try:
...
def confirm_DCconnection(dc_describe_response):
    input_data = global_input_data
    client = boto3.client('directconnect')
    default_connection = None
    default_connection_name = str(input_data['virtual_int_detail']['dc_name'])
...
#Creating DCG with random name
def create_direct_connect_gw():
    input_data = global_input_data
    client = boto3.client('directconnect')
...
#Accepting DirectConnect Connections
describe_connections = describe_connections()
default_connection = confirm_DCconnection(describe_connections)

#Creating Virtual Private Gateway (VPG)
vpn_gw_creation_resp = create_vpn_gateway()

#Attaching VPG to VPC
attach_vpn_gateway(vpn_gw_creation_resp)

#Creating DirectConnect Gateway
dc_gw_creation_resp = create_direct_connect_gw()

#Associating DirectConnect GW and Virtual Private GW
